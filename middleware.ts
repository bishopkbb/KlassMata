...
export default withAuth(
  function middleware(req) {
    ...
    const teacherRoles = [UserRole.teacher, UserRole.admin, UserRole.super_admin]
    const studentRoles = [UserRole.student, UserRole.teacher, UserRole.admin, UserRole.super_admin]
    ...
    if (pathname.startsWith('/api/super/') && userRole !== UserRole.super_admin) {
      return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
    }
...
export const config = {
  matcher: [
    '/((?!api/auth|api/register|_next/static|_next/image|favicon.ico|public|auth/signin|auth/signup|$).*)',
  ],
}
