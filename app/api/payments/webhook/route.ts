...
async function processSubscription(payment: any) {
  ...
  try {
    ...
    if (existingSubscription) {
      ...
      console.log(
        `Extended subscription for school ${payment.schoolId} until ${newEndDate}`,
      );
    } else {
...
function detectPaymentProvider(body: any, headers: Headers): string {
  ...
  if (
    body.event &&
    (body.event.includes("charge") || body.event.includes("transfer"))
  ) {
    return "flutterwave";
  }
...
// REMOVED 'export' keyword - this is now a private helper function
async function handleFailedPayment(paymentRef: string, reason: string) {
  try {
    const payment = await prisma.payment.findUnique({
